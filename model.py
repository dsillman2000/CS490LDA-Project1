...
def new_stage_model(next_stage_size=1, n_layers=2, n_neurons=32, **kwargs):
    """
    ...
    for l in range(n_layers):
        x = keras.layers.Dense(n_neurons, activation='relu',kernel_initializer='identity')(x)
    outp = keras.layers.Dense(next_stage_size, activation='relu',kernel_initializer='identity')(x)
    stage_model = keras.Model(inp, outp)
    stage_model.compile(**kwargs)
    return stage_model
    
def build_fit_rmi_model(dataset, stages=(1,2,3), n_layers=2, n_neurons=32, n_epochs=10, verbose='auto', **kwargs):
    """
    ...
    for i in range(M):
        for j in range(stages[i]):
            ...
            nn = new_stage_model(next_stage_size=1, n_layers=n_layers, n_neurons=n_neurons, **kwargs)
